...
if __name__ == "__main__":
    ...
    loaded_model, probabilities = load_model(timeframe)
    loss, accuracy = loaded_model.evaluate(x, y_test, verbose = 0)
    print('Test loss:', loss) 
    print('Test accuracy:', accuracy)

    y_hat = np.round(loaded_model.predict(x)).astype(np.int32)  
